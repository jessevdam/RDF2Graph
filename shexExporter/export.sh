...
rm -r ./temp
mkdir temp
tdbquery --loc $project --query queries/all.txt --results N3 > temp/temp1.n3
tdbquery --loc $project --query queries/createRoot.txt --results N3 > temp/temp2.n3
...
while [[ $(tdbquery -loc ./temp/tempdb/ --query queries/removeSubClassOfMeshDone.txt | grep '[01]' |sed 's/.*\([01]\).*/\1/') -eq 1 ]] ; do
  tdbupdate -loc ./temp/tempdb --update queries/removeSubClassOfMesh.txt
  echo "."
done
